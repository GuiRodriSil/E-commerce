<template>
  <div class="flex min-h-screen items-center justify-center bg-slate-100 px-4 py-10">
    <div class="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-soft">
      ...
      <h1 class="text-3xl font-black text-slate-900">Entrar</h1>
      ...
      <p class="mt-6 text-center text-sm text-slate-600">
        Não tem conta?
        <router-link to="/register" class="font-semibold text-slate-900 hover:text-slate-700">Criar conta</router-link>
      </p>
    </div>
  </div>
</template>
...
const validate = () => {
  ...
  if (!form.value.password.trim()) {
    errors.value.password = 'Informe sua senha.'
    valid = false
  }
